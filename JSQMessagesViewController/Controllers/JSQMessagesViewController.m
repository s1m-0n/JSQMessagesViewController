...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ((int)[[UIScreen mainScreen] nativeBounds].size.height == 2436) {
        self.bottomLayoutDefaultValue = 30;
        self.toolbarBottomLayoutGuide.constant = 30;
    } else {
        self.bottomLayoutDefaultValue = 0;
    }
    self.toolbarHeightConstraint.constant = self.inputToolbar.preferredDefaultHeight;
...
{
    if (![self.inputToolbar.contentView.textView isFirstResponder] && self.toolbarBottomLayoutGuide.constant == self.bottomLayoutDefaultValue) {
        return;
    }
    
    CGFloat heightFromBottom = CGRectGetMaxY(self.collectionView.frame) - CGRectGetMinY(keyboardFrame);
    
    heightFromBottom = MAX(self.bottomLayoutDefaultValue, heightFromBottom);
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            if ([UIDevice jsq_isCurrentDeviceBeforeiOS8]) {
                ...
                [UIView animateWithDuration:0.0
                                 animations:^{
                                     [self jsq_setToolbarBottomLayoutGuideConstant:self.bottomLayoutDefaultValue];
                                 }];
